...
class ModelManager:
    ...
    def transcribe(self, audio_path: str) -> str:
        ...
        with torch.no_grad():
            # This returns a list of results (strings or objects)
        ...
        # 1. If it's already a clean string, return it
        if isinstance(result, str):
            return result
            
        # 2. If it's a NeMo Hypothesis object, extract the 'text' field
        if hasattr(result, 'text'):
            return result.text
            
        # 3. Debug Fallback: Convert whatever it is to a string so we can see it
        print(f"DEBUG: Transcription returned type {type(result)}")
        return str(result)
    ...
    def generate(self, user_input: str, history: Optional[List[Dict]] = None) -> str:
        ...
        # Add history
        if history:
            messages.extend(history[-10:]) # Reduce context to keep it fresh
        else:
        ...
        # We strip the <think> tags from the history so the model doesn't get confused next turn
        clean_response_for_history = re.sub(r'<think>.*?</think>', '', full_response, flags=re.DOTALL).strip()
        if not clean_response_for_history:
             clean_response_for_history = full_response # Fallback if everything was a thought

        # Update history with the CLEAN version
        self.conversation_history.append({"role": "user", "content": user_input})
        self.conversation_history.append({"role": "assistant", "content": clean_response_for_history})
        
        # Return the FULL version (with tags) to the UI so it can parse them
        return full_response

    def synthesize(self, text: str, speaker_id: str = "en_0") -> bytes:
        """Synthesize speech with smart chunking for long text."""
        import numpy as np
        import io
        ...
        # Silero crashes if text > ~900 chars. We split by sentences to be safe.
        max_chars = 800
...
async def chat_with_speech(request: ChatRequest):
    ...
    try:
        ...
        # --- NEW: Remove <think> blocks for audio ---
        tts_text = re.sub(r'<think>.*?</think>', '', response_text, flags=re.DOTALL)
        # 2.1  Create a clean version for TTS
